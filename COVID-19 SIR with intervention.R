# SIR with intervention - COVID-19 Germany
...
# Works with Ebola and Covid data
a <- CovidGermany
C <- a$New_cases #dat$inf
D <- a$New_deaths #dat$rep
n_days <- length(C)
ncases <- sum(C)
s0=83200000-4 # - Cumulative Cases
i0=4
r0=0 # 0 deaths
h=1
N = 83200000
...
S <- vector(mode="numeric", length = 182) #51
S[1]=s0 #  initial conditions for S
I <- vector(mode="numeric", length = 182) #51
I[1]= i0 #  initial conditions for I
R <- vector(mode="numeric", length = 182) #51
...
model{
  ...
  for(t in 2:n_days){
  if(t <= 55){ // intervention point is 55
        Beta[t]= beta;
      }
      else{
        Beta[t]= beta*exp(-q*(t-55));
      }
      S[t]= S[t-1] - C[t-1];
...
generated quantities {
  ...
  for(t in 56:n_days){
  EffReproduction[t] = beta*exp(-q*(t-55))/gamma; 
  }
}
