...
seir_mult_generator <- odin::odin({
  ...
  p_SE[] <- 1 - exp(-beta*I[i]/N[i]) # S to E
...
x_res_int <- x$run(0:2000)
par(mar = c(4.1, 5.1, 0.5, 0.5), las = 1)
matplot(x_res_int[, 1], x_res_int[, -1], xlab = "Time", ylab = "Number of individuals",
        type = "l", col = alpha(rep(seir_col, each = 500), 0.3), lty = 1, 
        mgp=c(3.2,0.4,0), ylim = c(0,600000), xlim = c(0,1500))
...
# Epidemic size - based on susceptable definition - s0 - s(end time)
5364499 - mean(x_res_int[2000, 2:501])
